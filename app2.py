...
def main():
     @st.cache
     def load_data():
         z = ZipFile("C:/Users/Allihamdulilahi/Desktop/invest_ideas/han/p7/default_risk.zip")
         data = pd.read_csv(z.open('default_risk.csv'),
                            index_col='SK_ID_CURR', encoding ='utf-8')
         z = ZipFile("C:/Users/Allihamdulilahi/Desktop/invest_ideas/han/p7/X_data.zip")
         sample = pd.read_csv(z.open('X_data.csv'),
                              index_col='SK_ID_CURR', encoding ='utf-8')
         description = pd.read_csv("C:/Users/Allihamdulilahi/Desktop/invest_ideas/han/p7/features_description.csv",
                                   usecols=['Row', 'Features'], index_col=0, encoding= 'unicode_escape')
         #target = data.iloc[:, -1:]
         target = pd.read_csv(dirpath+"TARGET.csv")
         #target = data.iloc[:, -1:]
     ...
     def load_probabilities(sample, id, clf):
           ...
           df_prob['greater'] = ['Worse' if x > score else 'Equal or Better' for x in df_prob['Default Probability']]
           #percount = df_prob['greater'].sum()
                      
           position=df_prob[X.index == int(id)].index[0]    #1st condition fullfilled 
           
           resultdf=df_prob.loc[position-4:position+4,:] 
    ...
    """
     ...
     st.markdown("<u>List of the 8 files closest to this Customer default profile :</u>", unsafe_allow_html=True)
     prediction2, prevd = load_probabilities(sample, chk_id, clf)
     #st.dataframe(load_probabilities(sample, chk_id, clf))
     st.dataframe(prediction2)
         
     # Income distribution plot
     fig, ax = plt.subplots(figsize=(10, 5))
     sns.histplot(prevd['greater'], edgecolor= 'k', color="goldenrod")
     #sns.histplot(prevd['greater'], edgecolor= 'k', color="goldenrod", bins=2)
     ax.set(title='Default Probability Relative to Sample', xlabel='', ylabel='Count')
     st.pyplot(fig)
